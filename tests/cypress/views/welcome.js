...
const consolePublic = (token) => {
    return cy
        .request({
            url:
                apiUrl +
                '/api/v1/namespaces/openshift-config-managed/configmaps/console-public',
            headers: {
...
export const welcomePage = {
    ...
    shouldExist: () => {
        ...
        cy.get('.welcome--svcs')
            .should('contain', 'Go to Overview')
            .and('contain', 'Go to Clusters')
            .and('contain', 'Go to Applications')
            .and('contain', 'Go to Governance and risk')
    },
    validateSvcs: () => {
    ...
    validateConnect: () => {
        cy.get('[target="dev-community"]').should(
            'have.prop',
            'href',
            'https://www.redhat.com/en/blog/products'
        )
        cy.get('[target="support"]').should(
            'have.prop',
            'href',
...
export const applicationPage = {
    shouldExist: () => {
        cy.get('.pf-c-nav__item').contains('Overview')
        cy.get('.pf-c-nav__link').contains('Advanced configuration')
    },
}

export const grcPage = {
    shouldExist: () => {
        cy.get('.secondary-header', {timeout: 10000}).should(
            'contain',
            'Governance and risk'
        )
    },
...
export const leftNav = {
    ...
    goToOverview: () => {
        cy.get('.pf-c-nav__link').contains('Overview').click()
        overviewPage.shouldExist()
    },
    goToClusters: () => {
        leftNav.openMenu()
    ...
    goToApplications: () => {
        cy.get('.pf-c-nav__link').contains('Applications').click()
...
export const userMenu = {
    openApps: () => {
        ...
        cy.get('#applications a.dropwdown-content-items')
            .should('have.attr', 'href')
            .and('contain', 'console-openshift-console')
    },
    openSearch: () => {
    ...
    openTerminal: () => {
        ...
        cy.get('#acm-kui-dropdown #terminal-dropdown-content li')
            .should('be.visible')
            .and('have.length', 2)
            .then((c) => {
                cy.wrap(c)
                    .get('#new-tab-terminal a')
                    .should('have.prop', 'href', Cypress.config().baseUrl + '/kui')
    ...
    openInfo: () => {
        ...
        cy.get('#acm-info-dropdown #info-dropdown-content li')
            .should('be.visible')
            .and('have.length', 2)
        ...
        cy.getCookie('acm-access-token-cookie')
            ...
            .then((token) => {
                oauthTokenEndpoint(token.value).then((endpoint) => {
                    cy.get('#acm-user-dropdown #acm-user-dropdown-content li')
                        .should('be.visible')
                        .and('have.length', 2)
                        .get('#configure-client a')
                        .should('have.prop', 'href', endpoint + '/request')
                        .and('have.attr', 'target')
                        .and('match', /_blank/)
                })
            })
        cy.get('.navigation-container #acm-user-dropdown').click()
        cy.get('#acm-user-dropdown #acm-user-dropdown-content li').should(
            'be.not.visible'
        )
    },
}
