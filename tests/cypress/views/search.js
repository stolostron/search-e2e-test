...
export const searchPage = {
  ...
  whenDeleteNamespace: (namespace, options) => {
    ...
    var deleteFn = () =>
      searchPage.whenDeleteResourceDetailItem('namespace', namespace)
  ...
  shouldVerifyManagedClusterPodsAreRunning: (name) => {
    ...
    cy.get('.react-tags__suggestions ul#ReactTags')
      .children()
      .then((elem) => {
        if (elem.length <= 1) { // If the namespaces aren't returned, there's a good chance the cluster resources has been deleted.
